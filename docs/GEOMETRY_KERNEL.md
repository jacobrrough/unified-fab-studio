...
3. **Phase 3 (baseline for kernel post-ops):** After extrude/revolve/loft, optional ordered **`postSolidOps`** from `part/features.json` → `kernelOps` (rows with **`suppressed: true`** stay in the file for ordering but are **dropped** before Python sees them), applied in CadQuery before STEP/STL export:
   ...
   - **`boolean_combine_profile`** — second-body combine from kernel `profiles[profileIndex]` extrude (`mode`: union/subtract/intersect, `extrudeDepthMm`, `zStartMm`, optional **`extrudeDirection`** `+Z` / `-Z` for extrude sign from the workplane at `zStartMm`).
   - **`split_keep_halfspace`** — axis-plane split keep-side surrogate: intersect with half-space (`axis` X/Y/Z, `offsetMm`, `keep` positive/negative). On successful build, manifest includes **`splitKeepHalfspace`** (diagnostic). When the discarded half has non-trivial volume, Python also writes **`splitDiscardedStepPath`** / **`splitDiscardedStlPath`** (same `sketchPlane` transform as the kept body; basename `kernel-part-split-discard.*`). Examples: [`features.split-halfspace.example.json`](../resources/sample-kernel-solid-ops/part/features.split-halfspace.example.json), [`features.split-halfspace-negative-offset.example.json`](../resources/sample-kernel-solid-ops/part/features.split-halfspace-negative-offset.example.json).
  ...
  - **`thread_wizard`** — modeled/cosmetic thread workflow (standard/designation/class metadata, hand, starts). Modeled path follows a 3D helix at mean radius with ball-chain (sphere) cuts, with short-cylinder fallback if `sphere()` is unavailable — **approximation**, not a certified thread form.
   ...
   - **`pattern_path`** — path instance copies sampled along sketch polyline path points. Default **translation-only** along XY. Optional **`alignToPathTangent`** rotates each copy about **+Z** through the path start so local **+X** follows the path tangent at the sample (MVP). Optional **`closedPath`** adds the closing edge from the last point to the first when they differ (requires ≥3 points); uniform spacing uses total perimeter including that edge.
...
- **Success:** `{ "ok": true, "stepPath": "<abs>", "stlPath": "<abs>", "loftStrategy"?: "<tag>", "splitDiscardedStepPath"?: "<abs>", "splitDiscardedStlPath"?: "<abs>", "loftGuideRailsKernelMode"?: "marker" | "sketch_xy_align" }` — `loftStrategy` only when `solidKind` is `loft` and a strategy was chosen; split-discard paths only when a `split_keep_halfspace` discard body was exported. Optional payload **`stlMeshAngularToleranceDeg`** tightens STL tessellation (main + discard + inspect mesh when rebuild uses the same payload).
...
## Stretch deferrals (product vision)

**Rib, web, and selective fillet** as first-class `kernelOps` are **not shipped** in this batch: CadQuery/OCC scope for stable rib/web APIs and UI-driven selective edge bundles needs a dedicated milestone. Use existing **`fillet_select` / `chamfer_select`** (axis buckets) and **`plastic_rule_fillet`** where applicable; track broader **`so_rib` / `so_web`** catalog rows in [`PARITY_REMAINING_ROADMAP.md`](PARITY_REMAINING_ROADMAP.md).

## CAM and job documentation (cross-reference)

Kernel STEP/STL and **drawing export** (`project_views.py` mesh edges + optional convex hull) are separate from **manufacture CAM** (`cam:run`, `engines/cam/`). For operator checks on **setup-driven cut params**, **stock box on `cam:run`**, **4-axis** hints, **voxel sim** tiers, and **setup sheet** HTML from Shop or Manufacture, see the **CAM / manufacture** and **Manufacture sim** rows in [`VERIFICATION.md`](VERIFICATION.md) and [`engines/cam/README.md`](../engines/cam/README.md).
